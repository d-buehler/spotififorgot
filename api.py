...
def spotify_login(conf):
    ...
    creds = ':'.join([
        conf.get('API', 'client_id'), 
        conf.get('API', 'client_secret')
    ])
...
def searchTracks(conf, trackName, limit=50, removeSingles=True, debug=False):
    ...
    headers =  {
        'Authorization': ' '.join([creds['token_type'], creds['access_token']])
    }
    if debug:
    ...
    if removeSingles:
        searchResults = [ track for track in searchResults if track['album']['album_type'] != 'single' ]
