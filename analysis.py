...
AggregatedAlbumStats = namedtuple(
    "AggregatedAlbumStats",
    [
        "albumName",
        "nrQueriesMatched",
        "maxSimilarity",
...
def getAlbumStats(userTrackName, queryResults, debug=False):
    ...
    for track in queryResults:
        ...
        artists = ",".join(
            [
                artist.get("name", "No Artist Name")
                for artist in track.get("artists", [])
            ]
        )

        if albumName not in resultsAlbumStats.keys():
            resultsAlbumStats[albumName] = AlbumQueryStats(
                userTrackName=userTrackName,
            ...
            resultsAlbumStats[albumName].updateStats(
                userTrackName=userTrackName,
                querySimilarity=querySimilarity,
                releaseDate=releaseDate,
                queriedTrackPopularity=trackPopularity,
...
def updateAllStats(thisAlbumStats, userInput, allAlbumStats):
    ...
    """
    newAlbums = [
        album for album in thisAlbumStats.keys() if album not in allAlbumStats.keys()
    ]
...
def getTopAlbums(allAlbumStats, nrAlbums):
    ...
    topAlbums = sorted(
        ...
        reverse=True,
        key=attrgetter(
            "nrQueriesMatched", "maxSimilarity", "maxPopularity", "releaseDate"
        ),
    )
