...
def printTopAlbums(topAlbums):
    ...
    for i, albumStats in enumerate(topAlbums):
        albumStr = "'{}' by {}, released {} (Matched {} Queries, Max Similarity of {}, Max Popularity of {})".format(albumStats.albumName,
                                                                                                                     albumStats.artists,
...
def run(conf, debug=False):
    ...
    while True:
        ...
        try:
            queryResults[userInput] = searchTracks(conf, userInput, removeSingles=False, debug=debug)
        except InvalidTrackName as e:
            print(e.message)
            continue
        
        if debug:
            print("{} tracks in results for '{}'".format(len(queryResults[userInput]), userInput))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config",
                        ...
                        default=os.path.join(os.path.dirname(os.path.realpath(__file__)), "config/base.cfg"))
    
    args = parser.parse_args()
    
    print(figlet_format('Spotif... I Forgot.', font='roman'))
    print("Album Finder\n")
    print("Enter songs from that album you kinda-sorta-but-don't-really remember, and see if the results jog your memory!\n")
